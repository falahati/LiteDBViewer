            ...
            foreach (var item in _document.RawValue.OrderBy(pair => pair.Key))
            {
                ...
                switch (item.Value.Type)
                {
                    ...
                    case BsonType.DateTime:
                        itemString = item.Value.AsDateTime.ToString("yyyy-MM-dd HH:mm:ss.fff");
                        contextMenu = new ContextMenu();
                        contextMenu.MenuItems.Add(new MenuItem("View String",
                            (o, args) => new StringViewForm(item.Value.AsDateTime.ToString("yyyy-MM-dd HH:mm:ss.fff")).ShowDialog(this)));
                        break;
                    default:
